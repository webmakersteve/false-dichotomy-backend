...
module.exports.loginUserBnet = function makeLoginUseHandler(options) {
  ...
  return async (ctx) => {
    ...
    if (query && query.redirect) {
      newRedirectUri = querystring.unescape(query.redirect);
    }
...
module.exports.loginCallback = function makeLoginCallback(options) {
  ...
  return async (ctx) => {
    ...
    try {
      ...
      const returnV = await ctx.services.dynamo.putUser('discord', user, accessToken);
      jwt = ctx.services.discord.getJwt(returnV);
    } catch (e) {
      ctx.status = 400;
      ctx.type = 'application/json';
      ctx.body = JSON.stringify({
